...
describe('sortClassNames', () => {
  it('sort classnames into plugin groups', () => {
    ...
    ['bg-black', 'fill-pink', 'white', 'bg-white', 'pink'].forEach(
      className => {
  ...
  it('ignores className modifiers when sorting', () => {
    ...
    const testClassesWithModifiers = mdIntegerClassNames.concat(
      lgLengthUnits,
      hoverColorClassNames
    );

    const sortedClassNames = sortClassNames(
      testClassesWithModifiers,
      pluginRegexes
    );
