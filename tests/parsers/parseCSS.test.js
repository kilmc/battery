...
describe('determineValue', () => {
  describe('determineValueType', () => {
    it('determines the value type of a srcValue', () => {
      ...
      expect.assertions(2);
      expect(
        determineValueType(keywordTestValue, justifyContentConfig, config)
      ).toEqual('keyword');
      expect(
        determineValueType(integerTestValue, zIndexConfig, config)
      ).toEqual('plugin');
    });
  });
  ...
  describe('formatPluginValue', () => {
    ...
    describe('lookup type', () => {
      it('converts a srcValue to a formatted pluginValue', () => {
        ...
        expect(
          formatPluginValue('#000000', backgroundColorConfig, config)
        ).toEqual('black');
        expect(
          formatPluginValue('#FFFFFF', backgroundColorConfig, config)
        ).toEqual('white');
      });
  ...
  it('returns an error string if the value could not be matched', () => {
    ...
    expect(determineValue(testValue, borderConfig, config)).toEqual(
      "[no value found for '1px solid color(grey-500)']"
    );
  });
});
...
describe('declarationsToClassName', () => {
  it('converts a simple declaration to a className', () => {
    ...
    expect(declarationsToClassName(testDeclarations, config)).toEqual([
      'justify-end'
    ]);
  });
});
...
`;
    expect(styleBlockToClassNames(testStyleBlock, config)).toEqual({
      '.project-header--show': ['block']
    });
  });
...
`;
    expect(styleBlockToClassNames(testStyleBlock, config)).toEqual({
      '.project-info': ['flex', 'justify-end', 'z10', 'bg-black', 'white']
    });
  });
});
