...
describe('getKeywordClassObjs', () => {
  ...
  it('returns classObjects for each matching className passed', () => {
    expect(getKeywordClassObjs(classNames, classObjects)).toEqual({
      'bg-contain': { 'background-size': 'contain' },
      flex: { display: 'flex' },
      'm-auto': { margin: 'auto' }
    });
  ...
  it('ignores modifiers in the className', () => {
    expect(getKeywordClassObjs(classNamesWithModifiers, classObjects)).toEqual({
